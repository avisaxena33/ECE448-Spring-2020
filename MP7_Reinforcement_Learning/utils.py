...
def hyperparameters():
    ...
    parser.add_argument('--epsilon_min', type=float, default=0.01, help='minimum exploration parameter')
    parser.add_argument('--epsilon_decay_factor', type=float, default=.995, help='exploration decay parameter')
...
def rollout(env: gym.Env, policies: policies.QPolicy, episodes: int, temp: float, render: bool = False):
    ...
    for itrnum in range(episodes):
        ...
        while not done:
            ...
            if str(policies) == 'DQNPolicy':
                action = np.random.choice([0, 1], p=pi) #TODO: fill in this line
            else:
